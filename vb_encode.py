...
def build_frame(data: bytearray, width: int, height: int, count: int, pixel_size: int, palette_size: int):
...
def build_frames(input_file: str, width: int, height: int, pixel_size: int, palette_size: int, threads: int, strong_ecc: bool):
    """Encodes any file into images."""
    # first frame will always be metadata
    ecc_bytes_count = 16 if strong_ecc else 8
    ...
    meta_bytes += ecc_bytes_count.to_bytes(1, byteorder="big")
    ...
    frames.append(build_frame(meta_ecc.encode(meta_bytes), width, height, 0, pixel_size, palette_size))
    ...
    def work(num: int):
        ...
        #new_bytes = file.read(content_bytes_per_frame)
        build_frame(bytearray_to_encode, width, height, num, pixel_size, palette_size)
    cur_frame = 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--crf", metavar="crf", type=int, help="Quality of the video (constant rate factor). *Lower* values will increase quality (therefore less compression artifacts) and file size. Might not work with every video codec.", default=22)
    ...
    parser.add_argument("--strong_ecc", dest="strong_ecc", action="store_true", default=False, help="Uses 16 instead of 8 bytes for error correction. Encoding and Decoding will take marginally longer and filesize will be bigger.")
    ...
    print("\"Bandwidth\" of the video will be approx. %dB/s." % math.floor((args.width * args.height * args.fps) / math.log(256, args.color_palette) / (args.pixel_size ** 2) / (BLOCK_SIZE / (BLOCK_SIZE - (16 if args.strong_ecc else 8)))))
    
    if not os.path.exists("tmp"):
    	os.mkdir("tmp")
    
    frames = build_frames(args.input, args.width, args.height, args.pixel_size, args.color_palette, args.threads, args.strong_ecc)
